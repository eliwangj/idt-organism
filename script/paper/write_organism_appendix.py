"""DEPRECATED. Do not run: it would overwrite the paper's organism numbers.

The IDT paper's `appendix/organism_numbers.tex` is now generated by Ian's
detector repository (`script/paper/write_data_appendix.py`) from HIS OWN
reproduction: he ran this organism through his registered test with his judge
seat, and every number in the appendix comes from that run, not from the
artifacts here. Regenerating the file from this script would replace a
reproduction with the original author's numbers, which is the wrong direction.

Kept for the record of how the first version of the appendix was numbered, and
because `signed()`, `pval()`, and `count()` document the paper's formatting
conventions. If a fresh comparison against Ian's numbers is ever wanted, run
this with --out to a scratch path, never to the paper.
...
def main() -> None:
    ...
    if "appendix" in args.out.parts:
        raise SystemExit(
            "refusing to write into a paper appendix directory: that file is now "
            "generated from Ian's reproduction. Use a scratch --out path."
        )
